<x-layout title="Update Water Bill | VoltTrack">
    <div class="max-w-5xl mx-auto p-8 bg-slate-50 min-h-screen">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <form method="POST" action="{{ route('admin.bills.update', $bill->id) }}" class="lg:col-span-2 bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-sm" onsubmit="if (!confirm('Update this water bill?')) return false; if (this.querySelector('[name=status]').value === 'Paid') return confirm('Confirm again: set this water bill to Paid?'); return true;">
                @csrf
                @method('PATCH')
