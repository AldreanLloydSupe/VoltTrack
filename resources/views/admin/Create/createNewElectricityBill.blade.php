<x-layout title="Create Electricity Bills | VoltTrack">
    <div class="max-w-4xl mx-auto p-8 bg-slate-50 min-h-screen">
        <div class="mb-8">
            <p class="text-xs font-bold text-slate-500 uppercase tracking-wider">Billing > Electricity</p>
            <h1 class="text-3xl font-black text-[#001D4E] mt-2">Create Electricity Bill</h1>
            <p class="text-sm text-slate-500 mt-2">Create or update an Electricity bill for one selected resident.</p>
        </div>
        ...
        <form method="POST" action="{{ route('admin.Create.storeNewElectricityBill') }}" class="bg-white border border-slate-200 rounded-2xl p-6 space-y-6 shadow-sm">
            ...
            <div>
                <label class="block text-sm font-semibold text-slate-700 mb-2">Resident</label>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Previous Reading (kWh)</label>
                    <input type="number" step="0.01" min="0" name="previous_reading" value="{{ old('previous_reading', 0) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2" required>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Current Reading (kWh)</label>
                    <input type="number" step="0.01" min="0" name="current_reading" value="{{ old('current_reading', 0) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Price Per Unit</label>
                    <input type="number" step="0.01" min="0" name="price_per_unit" value="{{ old('price_per_unit', 0) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2" required>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-2">Service Fee</label>
                    <input type="number" step="0.01" min="0" name="service_fee" value="{{ old('service_fee', 0) }}" class="w-full rounded-lg border border-slate-300 px-3 py-2">
                </div>
                <div>
            ...
            <div class="flex items-center gap-3 pt-2">
                <a href="{{ route('admin.billingHistory') }}" class="px-5 py-2.5 rounded-lg bg-slate-200 text-slate-700 font-semibold">Cancel</a>
                <button type="submit" class="px-5 py-2.5 rounded-lg bg-[#001D4E] text-white font-semibold">Create Electricity Bill</button>
            </div>
        </form>
    </div>
</x-layout>
